...
- $A_1, A_2, \dots, A_K$ を昇順に並び替えた列を $B_K$ とすると、 $K$ 個目の問題の解は $\frac{1}{K^2}\sum_{i=1}^{K}(2i-1)B_{K,i}$
- $f(K) = \sum_{i=1}^{K}(2i-1)B_{K,i}$ とし、 $f(K)$ から $f(K+1)$ を作ることを考える
  - 列 $B_K$ に対して $A_{K+1}$ を昇順を保ったまま挿入することを考える。このとき $A_{K+1}$ が $j$ 番目に挿入されるとすると
    - $i < j$ に対し $B_{K+1, i} = B_{K, i}$
    - $i > j$ に対し $B_{K+1, i} = B_{K, i-1}$
  - よって以下が成り立つ
$$\begin{align*}
  f(K+1) &= \sum_{i=1}^{j-1} (2i-1)B_{K+1,i} + (2j-1)A_{K+1} + \sum_{i=j+1}^{K+1} (2i-1)B_{K+1, i} \\
  &= \sum_{i=1}^{j-1} (2i-1)B_{K,i} + (2j-1)A_{K+1} + \sum_{i=j}^{K} (2i+1)B_{K, i} \\
  &= f(K) + (2j-1)A_{K+1} + 2\sum_{i=j}^{K} B_{K, i}
\end{align*}$$

- $j$ は $B_K$ 内の $A_{K+1}$ より小さい要素の個数であることから、 $\sum_{i=j}^{K} B_{K, i}$ は $B_K$ 内の $A_{K+1}$ より小さくない要素の和といえる
  - よって「 $A_{K+1}$ より小さい要素の個数」 「 $A_{K+1}$ より小さくない要素の和」のそれぞれを`atcoder::fenwicktree`で管理すればOK
- 計算量は $M = \max A$ として $O(N \log M)$

## [ARC134 C - The Majority](https://atcoder.jp/contests/arc134/tasks/arc134_c)
- 場合の数を丁寧に数え上げるだけの問題
- ボールの区別はつかないが箱の区別はつくため、基本的にはボールの種類ごとに重複組合せを計算すればよい
- ボール $1$ が過半数を占める条件は以下に言い換えられる
  - あらかじめ各箱にボール $1$ を1個入れておく
  - ボール $i > 1$ を箱にいれるときは必ずセットでボール $1$ も入れる
- ボール $i > 1$ の入れ方は ${}_{a_i}H{K}$ 通り、ボール $1$ の入れ方はボールの合計数を $S$ として ${}_{a_i - K - (S - a_i)}H_{K}$ 通りであり、この積が解になる
  - 勿論 $a_i - K - (S - a_i) < 0$ となってしまう場合は解が $0$ になる
- $a_i$ が巨大なのであらかじめ階乗のテーブルを用意するのではなく、愚直に $n(n-1) \dots (n-r+1)$ を計算したほうが早い

## [ABC175 E - Picking Goods](https://atcoder.jp/contests/abc175/tasks/abc175_e)
- どうみても（行、列、その行で何個拾ったか）の3要素DPですありがとうございました
- 先頭から順に何の工夫もなくDPするだけなのになぜこの難易度（1554）に……？
  - ちょっと前にやった同コンテストのD - Moving Pieceのほうがよほど難しく感じる……

## [ABC129 E - Sum Equals Xor](https://atcoder.jp/contests/abc129/tasks/abc129_e)
- $a + b = a \oplus b$ が成立する必要十分条件は $a$ と $b$ の $i$ ビット目がともに $1$ になるような $i$ が存在しないこと
  - すなわち $a + b \leq L$ を無視すれば各ビットごとに $(0, 0), (0, 1), (1, 0)$ の3通りがあるので、3のビット数乗個の解があることになる
- $L$ の $i$ ビット目以下を全て $0$ にしたものを $X_i$ 、 $j > i$ で $L$ の $j$ ビット目が $1$ となる $j$ の個数を $C_i$ をおくと、
  - 区間 $[1, L)$ は区間 $[X_i, X_{i-1})$ の足し合わせで過不足無く表せる
  - $L$ の $i$ ビット目が $0$ のとき、 $X_i = X_{i-1}$ なので区間 $[X_i, X_{i-1})$ は空である
  - $L$ の $i$ ビット目が $1$ のとき、区間 $[X_i, X_{i-1})$ での解は $2^{C_i}3^{i-1}$
- あとは $a+b=L$ となる組み合わせ $2^{C_0}$ を足せばOK

## [ABC258 E - Packing Potatoes](https://atcoder.jp/contests/abc258/tasks/abc258_e)
- じゃがいも $W_i$ から始まる箱の次がじゃがいも $W_j$ から始まるとすると、 $j$ は $i$ の関数と言えて探索ができる
  - 累積和を二分探索するのが最も簡単か
- ↑のルールに従って頂点 $i$ から頂点 $j$ に有向辺を張ると、Functional graphを成す
  - すなわちサイクルに含まれない長さ $L \geq 0$ の部分と、長さ $C \geq 1$ からなるサイクルの2つの部分から成る
- …というのはすぐにわかったのだが、デバッグで死ぬほど時間を溶かした
  - $i$ や $j$ をそのまま頂点にしてグラフを作ったが、実際に箱に入るじゃがいもの数を頂点にしたほうが色々と楽であることになかなか気付けなかった

## [ABC336 E - Digit Sum Divisible](https://atcoder.jp/contests/abc336/tasks/abc336_e)
- 桁和は $1$ 以上 $9 \cdot 14 = 126$ のいずれかであるから、これを固定して考えるのがよさそう
  - 以降桁和を $p$ とする
- 整数 $n$ の末尾に1桁の数 $x$ を付け加えると新たな数は $10n + x$ であり、 $n \bmod p$ がわかっていれば $(10n + x) \bmod p$ もわかる
  - すなわち整数を $p$ で割った余りで分類しながら上から桁DPを行える
- ただし $N$ 以下という制約があるため、（桁数、現在の桁和、 $p$ で割った余り）の3要素だけでは不十分である
  - $n$ が $N$ の上から $i$ 桁より小さい数のとき、 $x$ として何を選んでも $10n + x$ は $N$ の上から $i+1$ より大きくなることはない
  - $n$ が $N$ の上から $i$ 桁と等しいとき、 $10n + x$ として選べる $x$ は $N$ の上から $i+1$ 桁目の数以下に限られる
  - よって $n$ が $N$ の上から $i$ 桁より小さいか、あるいは等しいかの情報を加えて4要素DPをすればOK
- あとは $p = 1, 2, \dots, 126$ の順に↑のDPをやればよい
- 4要素DPを複数回ぶん回すとか狂気だな……流石制限時間10秒の問題ではある
  - 尤も、C++だと1000ms未満で終わるという

## [ARC197 C - Removal of Multiples](https://atcoder.jp/contests/arc197/tasks/arc197_c)
- 解の最大値を $N$ として、やるべきことはエラトステネスの篩っぽいことなのでその部分の計算量は $O(N \log N)$
- ただし $B_i$ 番目を答えるのにSegment treeがほしいので実際には $O(N (\log N)^2)$
- ここで $N$ がどれぐらいかを考える
  - $N$ が最大となるのは $A_i$ が素数の小さい順から満たされる場合であり、このとき最後のクエリの解は $Q + B_i = 2 \times 10^5$ 番目に小さい素数となる
    - $2 \times 10^5$ 番目に小さい素数は $2750161$ であり $10^6$ オーダーなので問題ないと判断できる
    - 探索しなくてもググればわかる時代ですねぇ……
- なお以下の2点を間違い`WA`
  - $N$ を $2 \times 10^5$ 番目ではなく $10^5$ 番目に小さい素数としてしまった
  - すでに消された $A_i$ をもう一度消そうとした
    - エラトステネスの篩が $O(N \log N)$ なのは $N + \frac{N}{2} + \frac{N}{3} + \dots = N \log N$ だからであり、極論 $A_i=2$ を連打されると $\frac{N}{2} +\frac{N}{2} +\dots $ となり $N \log N$ どころの騒ぎではなくなる

## [ARC176 B - Simple Math 4](https://atcoder.jp/contests/arc176/tasks/arc176_b)
- $M - K = 1$ のとき $2^M - 2^K = 2^K$ より $N \geq K$ なら解は $0$ 、そうでないなら $2^N \bmod 10$
- それ以外で $N < M$ のとき解は $2^N \bmod 10$
- 上記2つに当てはまらない場合、筆算で割り算することで以下がわかる
  - $i = 1, 2, \dots$ として $2^{N - iM + (i-1)K}$ の位に商 $1$ が立つ
    - $i$ 番目の商に対する余りは $2^{N - iM + iK}$
  - よって $N - iM + (i-1)K \geq 0$ を満たす最大の整数 $i_{\max} = \left\lfloor \frac{N-K}{M-K} \right\rfloor$ を用いて、 $2^N$ を $2^M - 2^K$ で割った余りは $2^{N-i_{\max}M+i_{\max}K}$ と表せる
...
## [ABC055 D - Menagerie](https://atcoder.jp/contests/abc055/tasks/arc069_b)
- 解を表す文字列を $T$ とする
- $s_i$ が `o` のときは $T[(i-1)..(i+1)]$ は `SSS`, `WSW`, `SWW`, `WWS` のいずれかで、そうでないときはそれ以外になる
  - これを言い換えると、 「 $s_i$ が`o`」「 $T_{i-1} = T_i$」の真偽が一致すれば $T_{i+1}$ が `S` になる
- よって以下の方針でいける
    1. 初期の $T$ として `SS`, `SW`, `WS`, `WW` の4通りを与える
    1. 4通りの $T$ のそれぞれに対して、 $i = 2, 3, \dots, N-1$ の順で以下を行う
        1. 「 $s_i$ が`o`」「 $T_{i-1} = T_i$」の真偽が一致すれば $T_{i+1}$ を `S` にする。しなければ `T` にする。
    1. $s_N, s_1$ を見て $T$ に矛盾がないか調べる

## [AGC041 B - Voting Judges](https://atcoder.jp/contests/agc041/tasks/agc041_b)
- $A_i$ を降順にソートし、問題 $i$ が採用されうるかどうかについて考える
  - $i \leq P$ は無条件でOK
  - $A_i + M < A_P$ となる $i$ は無条件でNG
  - それ以外の $i$ において
    - 問題 $P-1$ 以前と問題 $i$ にそれぞれ $M$ 人全員が投票し、残り $(V-P)M$ 票を問題 $i+1$ 以降に振り分けられるかどうかを考えたが
      - この考え方だと $M$ 票以上を獲得してしまう問題が出てくるためNG
    - 正しくは以下
        1.  問題 $P-1$ 以前と問題 $i$ 以降全てにそれぞれ $M$ 人全員が投票する
            - $i+1$ 問目以降に$+M$ 票されようが $i$ 問目も $+M$ 票されているので全く問題ない
        1. $P \leq j < i$ を満たす問題 $j$ に対して $A_i + M - A_j$ 人が投票する
        1. まだ票が余っていればNG、余っていなければOK
- 1問あたり $O(N)$ で探索でき、明らかに単調性があるため二分探索で全体 $O(N\log N)$ でいける

## [ABC222 E - Red and Blue Tree](https://atcoder.jp/contests/abc222/tasks/abc222_e)
- 1回の移動における最大辺数は $N-1$ なので、一連の操作で辿る延べ辺数は $(N-1)M \approx 10^5$
  - よって駒の動きを1つずつ舐めることは時間的に可能
- 代わりにどの辺を辿れば良いのかの判断についてはなるべく高速にしたい
  - ある頂点の親を得るのは事前にDFSしておけば $O(1)$ で可能
  - 頂点 $A_i$ と $A_{i+1}$ の最近共通祖先を $p$ とおくと、頂点 $A_i$ から $A_{i+1}$ の移動は「頂点 $A_i$ から $p$ 」 「頂点 $A_{i+1}$ から $p$ 」に分けられる
    - 最近共通祖先はダブリングを用いれば $O(\log N)$ で求められる
- 以上により一連の動きで駒が辺 $i$ を通過した回数 $C_i$ を $O(NM \log N)$ で網羅できる
- 駒の動いた回数の総和を $S$ とおくと、 $S = R+B$ より $R = \frac{S+K}{2}$
  - ただし $R$ が整数にならない場合または $0 \leq R \leq S$ に収まらない場合は解は $0$ になる
- あとは $C$ から0個以上の要素を選んで和を $R$ にする問題を解けば良い
  - 2要素DPが最も簡明
- 計算量は $O(NM \log N + NM^2)$
- （公式解説）木の辺数は $N-1$ だからLCAなど持ち出さなくとも毎回DFSで間に合うのか……

## [ARC178 B - 1 + 6 = 7](https://atcoder.jp/contests/arc178/tasks/arc178_b)
- 簡単のため $A_1 \leq A_2$ とする
- $A_2 = A_3$ のとき
  - 繰り上がりが起こらない
  - 解は領域 $10^{A_1-1} \leq X_1 < 10^{A_1}, 10^{A_2-1} \leq X_2 < 10^{A_2}, X_1 + X_2 < 10^{A_2}$ における格子点の数
    - すなわち ${\displaystyle \sum_{X_1 = 10^{A_1-1}}^{10^{A_1} - 1} \max \{ 10^{A_2} - 10^{A_2-1} - X_1, 0 \}}$
      - $A_1 < A_2$ のとき、 ${\displaystyle \sum_{X_1 = 10^{A_1-1}}^{10^{A_1} - 1} 9 \cdot 10^{A_2-1} - X_1}$
      - $A_1 = A_2$ のとき、 ${\displaystyle \sum_{X_1 = 10^{A_1-1}}^{9 \cdot 10^{A_1-1} - 1} 9 \cdot 10^{A_2 - 1} - X_1}$
- $A_2 + 1 = A_3$ のとき
  - 繰り上がりが起こる
  - 解は領域 $10^{A_1-1} \leq X_1 < 10^{A_1}, 10^{A_2-1} \leq X_2 < 10^{A_2}, X_1 + X_2 \geq 10^{A_2}$ における格子点の数
    - すなわち ${\displaystyle \sum_{X_1 = 10^{A_1-1}}^{10^{A_1} - 1} \min \{ 10^{A_2} - 10^{A_2-1}, X_1 \}}$
      - $A_1 < A_2$ のとき、 ${\displaystyle \sum_{X_1 = 10^{A_1-1}}^{10^{A_1} - 1} X_1}$
      - $A_1 = A_2$ のとき、 ${\displaystyle \sum_{X_1 = 10^{A_1-1}}^{9 \cdot 10^{A_1-1} - 1} 9 \cdot 10^{A_2 - 1} + \sum_{X_1 = 9 \cdot 10^{A_1-1}}^{10^{A_1}} X_1}$
- それ以外のとき
  - 不可能な組み合わせなので解は $0$
- 場合分けや境界の処理がうまくなりたいねぇ……毎度えげつない時間がかかる

## [ARC124 C - LCM of GCDs](https://atcoder.jp/contests/arc124/tasks/arc124_c)
- 忘れた頃にやってくる決め打ちパターン
- 題意より $X, Y$ として可能性があるのはそれぞれ $a_1$ の約数、 $b_1$ の約数に限られる
- $(X, Y)$ の候補の組み合わせ $(x, y)$ に対し、実際に $(x, y)$ が可能かどうかはカードパックを見ていけばわかる
  - 具体的には全ての $i$ に対し以下のいずれかが成り立てばよい
    - $a_i$ が $x$ の倍数かつ $b_i$ が $y$ の倍数
...
- とりあえず二分探索でチームの総合力を $x$ にできるかどうかを判定したい
- 人 $i$ に対してパワー・スピード・テクニック・知識・発想力のそれぞれが $x$ 以上であるかをビットパターンで表したものを $P_i(x)$ とおくと
  - 選ばれた3人 $i, j, k$ に対して $P_i(x) \ \mathrm{OR} \ P_j(x) \ \mathrm{OR} \ P_k(x) = 31$ となれば総合力を $x$ 以上にできると判断できる
- ↑の部分は（人、すでに選んだ人数、選ばれた人によるビットパターン）の3要素DPで判断した
  - 初め $N^2 \approx 10^7$ に引っ張られすぎて何故か $O(N^2)$ となる探索をしてしまった（当然`TLE`）
- 全体計算量はチーム人数を $M$ 、パラメータ数を $K = 5$ 、全パラメータの最大値を $X$ として $O(NM2^K \log X)$
- （公式解説） $P_i(x)$ が32通りの値にしかならないので、ここから重複を許さず3つ取り出して全パラメータをカバーできるかどうかを判断することができる
  - DPなんて要らんかったんや……

## [ABC170 E - Smart Infants](https://atcoder.jp/contests/abc170/tasks/abc170_e)
- 各幼稚園を表すデータとして、幼児のレートをキー、そのレートを持つ人数を値とする`map`（ただしキーの降順で整理）をSegment treeに載せてみたいが
  - `op(a, b)`で`map`のマージが必要になるので流石に厳しいと判断
- よって`map`はそのままでSegment treeはその幼稚園に所属する幼児のレート最大値のみを保持する方針にした
  - 転園手続きごとに`map`を更新してから、Segment treeの転出元、転入先に関する要素のみ`map`を参照して最大値を書き換えればOK
- 計算量は $O(Q \log N)$
- 多重集合相手に`multiset::erase()`が面倒でよく`map`を使っているが、流石に今回は`multiset`にすべきだったか……？

## [ABC231 F - Jealous Two](https://atcoder.jp/contests/abc231/tasks/abc231_f)
- **青木君に**プレゼント $i$ を、**高橋君に**プレゼント $j$ を渡すとして、 $A_i \leq A_j$ かつ $B_i \geq B_j$ となる $(i, j)$ の個数が解になる
- $A_i, B_i$ を $A_i$ の昇順→ $B_i$ の昇順でソートし、 $B_i$ の転倒数を数えるだけ……と思いきや
  - 等号があるため随所に差がある
- 以降 $i < j$ とする
- $A_i < A_j$ かつ $B_i = B_j$ のとき
  - これは転倒数を数える際に $B_j$ **以上**になる要素数を足していけばOK
- $A_i = A_j$ かつ $B_i < B_j$ のとき
  - 青木君が $i$ をもらうと嫉妬してしまう。むしろ $j$ を渡すべき状態になる
  - これではマズいので、初めのソート順を $A_i$ の昇順→ $B_i$ の**降順**に変更する
  - そうすれば $A_i = A_j$ なら $B_i > B_j$ が約束されるので、青木君に $i$ を渡すと嫉妬せず $j$ を渡すと嫉妬する状態にでき、転倒数の数え上げが使えるようになる
- $A_i = A_j$ かつ $B_i = B_j$ のとき
  - プレゼントの渡し方は $(i, j), (j, i)$ のいずれもOKとなるが、転倒数の数え上げではどちらか一方にしか対応できない
  - よって他方に対応するため、転倒数を数え上げた後に改めて $(A_i, B_i) = (A_j, B_j)$ となる $(i, j)$ の組を探す
    - ソートにより $(A_i, B_i)$ が等しい $i$ は必ず連続する1つの区間を成し、区間が異なれば $A_i$ か $B_i$ の少なくとも一方は異なる
      - 区間に含まれる要素数を $c$ として $c^2$ 通りが許されるが、転倒数の数え上げにより $c(c-1)/2$ 通りはすでに数えているので残りの $c(c+1)/2$ 通りを解に足しておけばOK
- 検索するとちらほらウェーブレット行列の名を目にしたが……これも習得しておくべきか……？

## [ABC228 E - Integer Sequence Fair](https://atcoder.jp/contests/abc228/tasks/abc228_e)
- ついにフェルマーの小定理が輝く時が来た！
- $M$ が $P$ の倍数でないとき、 $K^N$ を $P-1$ で割った商と余りを $q, r$ とおくと $M^{K^N} = M^{q(P-1)+r} = (M^{P-1})^q \cdot M^r \equiv M^r \pmod{P}$
- あとは $r$ などを計算するのみ
  - `atcoder/math`ファイル内の`atcoder::pow_mod`が最も簡明
- なお $M$ が $P$ の倍数のときをきちんと除外しないと`WA`を喰らうので注意
  - $K$ が $P-1$ の素因数をすべて持つとほとんどの場合 $r = K^N \bmod (P-1) = 0$ であり、このとき $M^r \bmod P = 1$ となってしまう

## [ABC284 F - ABCBAC](https://atcoder.jp/contests/abc284/tasks/abc284_f)
- 0-indexedで $T[i, i+N)$ と $T[0, i) + T[N+i, 2N)$ を逆にしたものが一致すればOK
- Rolling hashで以下のように一致するかどうかを判定した
    1. $T[0, N)$ の左からのハッシュを $L$ 、 $T[N, 2N)$ の右からのハッシュを $R$ とする
    1. $i = 0, 1, \dots, N-1$ の順に以下を行う
        1. ハッシュが一致していれば適切なものを出力して終わる
        1. $L$ から最も左の文字に相当するハッシュを削除する。すなわち $L \gets L - T_iB^{N-1}$
        1. $L$ を1文字分左へシフトする。すなわち $L \gets BL$
        1. $L$ の最も右に文字 $T_{N+i}$ を挿入する。すなわち $L \gets L + T_{N+i}$
        1. $R$ の左から $i$ 文字目に相当するハッシュを削除する。すなわち $R \gets R - T_{N+i}B^i$
        1. $R$ の左から $i$ 文字目に文字 $T_i$ を挿入する。すなわち $R \gets R + T_iB^i$
- ハッシュから引き算するときは結果が負にならないように留意
  - modを取るときに負数になって一致判定ができない
  - 負になっていたら正にするか、あらかじめ引く数をmod未満にするようにして、必ずmodを足しておくようにする
- 念のため3本のハッシュを取ったが、比較する組み合わせが常に1組のみだから1本でもいけた説
  - 参考：[ハッシュの衝突 - あなたは嘘つきですかと聞かれたら「YES」と答えるブログ](https://snuke.hatenablog.com/entry/2017/02/03/035524)
  - ハッシュが衝突しやすいのはいわゆる誕生日攻撃に相当するとき

## [ABC155 E - Payment](https://atcoder.jp/contests/abc155/tasks/abc155_e)
- 下の桁から貪欲に決めていきたい
- 下から $i$ 桁目を $D_i$ とすると、確実にお釣りをもらったほうが得なのが $D_i \geq 6$ のときで、確実にお釣りをもらわないほうが得なのが $D_i \leq 4$ のとき
- $D_i = 5$ のときはその桁だけでは判断がつかず、
  - $D_{i+1} \geq 5$ ならお釣りをもらったほうが得になり、そうでないならお釣りをもらわないほうが得になる
- $D_i = 5$ のときの条件を思いつくのに膨大な時間がかかってしまった……
  - こういうのを頭で考えようとせずに、2桁を全探索して条件を見出すような力が必要なんだろうなぁ

## [ARC111 B - Reversible Cards](https://atcoder.jp/contests/arc111/tasks/arc111_b)
- グラフを用意し、 $a_i, b_i$ を両端とする無向辺を張ると
  - 連結成分ごとにそれが木の場合は辺数、木でない場合は頂点数を足し合わせると解になる
    - Union-Find木で辺を張る前に両端の頂点が連結かどうかを調べ、連結ならばその頂点はサイクル上にある（or自己ループを持つ）と判断できる
      - 連結成分内にこのような頂点が1つでも存在すれば木でないと判断できる
- Union-Find木を使わずとも単なるDFSで行けたかもしれない……

## [ARC168 B - Arbitrary Nim](https://atcoder.jp/contests/arc168/tasks/arc168_b)
- 個数制限付きNim： $A_i \bmod (k+1)$ のXOR sumが $0$ なら後手勝ち、それ以外なら先手勝ち
  - 参考：[競プロの Nim 問題まとめ (ネタバレあり) #競技プログラミング - Qiita](https://qiita.com/kobae964/items/775c10877763a65a0328)
- 十分大きな $k$ に対して $A_i \bmod (k+1) = A_i$ であるから、そもそも $A_i$ のXOR sumが $0$ 以外なら解は $-1$
- そうでないとき、  $A_i \bmod (k+1)$ を変化させてXOR sumが $0$ でないようにできないかを考える
  - ↑より $k+1$ を $A_i$ の最大値より大きくしても意味はないので、 $k+1 = \max A$ としてみる
    - このとき $A_i = \max A$ を満たす要素に対して $A_i \bmod (k+1) = 0$ となり、それ以外に対しては相変わらず $A_i \bmod (k+1) = A_i$ となる
      - よって $A_i = \max A$ を満たす要素が奇数個ならXOR sumを $0$ 以外にできているので、解は $\max A - 1$
      - 偶数個の場合は等しい数値の偶数個のXOR sumは $0$ であるから変化していないことになる
        - 逆に言えば $k$ がいくらであってもXOR sumに何も関与しないと言えるので、 $A$ から $\max A$ と等しい要素を全て取り去って再びこの議論を行えばOK
          - 最終的に要素がなくなってしまったらどうあがいても後手必勝なので解は $0$ となる

## [ABC296 E - Transition Game](https://atcoder.jp/contests/abc296/tasks/abc296_e)
- 黒板から消される数から書かれる数の向きに有向辺を張ると、Functional graphになる
- 十分大きな $K$ が選ばれると最後に書かれる数はサイクル（自己ループ含む）上の頂点に限られることになる
  - よって各連結成分ごとにサイクル上の頂点の個数を数えて足し合わせればOK
- DFSでサイクルを検出するコードを書いたがどこか間違っていたらしく`WA`の嵐
  - サイクル上の頂点数を求めたいのなら強連結成分分解でできるんだから車輪の再発明はしない！

## [東京海上日動 プログラミングコンテスト2020 C - Lamps](https://atcoder.jp/contests/tokiomarine2020/tasks/tokiomarine2020_c)
- 操作を何度か行うと全ての $A_i$ が $N$ になりそれ以上変化しなくなる
- 試してみると初期 $A_i$ が全て $0$ の状態でも $N = 100$ で15回程度、 $N = 1000$ で20回程度で飽和したので $N = 2 \times 10^5$ でも大した回数にはならないと予想
- ただし流石に $O(N^2)$ では`TLE`するのでimos法で $O(N)$ に落としておく

## [ARC153 B - Grid Rotations](https://atcoder.jp/contests/arc153/tasks/arc153_b)
- ぐちゃぐちゃな移動になるかと思いきや、入力例と出力例を見ると実は意外と綺麗に並んでいる
  - よってよくよく眺めれば以下に気づく
- $Q$ 回の操作後 $A_{1,1}$ が $A_{x,y}$ に移ったとすると、
  - $Q$ が偶数のとき、 $A_{i,j}$ は $A_{i+x-1, j+y-1}$ へ移る
  - $Q$ が奇数のとき、 $A_{i,j}$ は $A_{i-x+1, j-y+1}$ へ移る
  - ただし、 $x = x' \pm H$ および $y = y' \pm W$ と表せるとき、マス $(x, y)$ と $(x', y')$ は同じマスを指すものとする
- よって後は $A_{1,1}$ がどこに行くかを $O(Q)$ で追えばよいだけ

## [ARC108 C - Keep Graph Connected](https://atcoder.jp/contests/arc108/tasks/arc108_c)
- 木は条件を満たせるのでDFSで適当に全域木を持ってくればOK
- 頂点 $1$ を根として子に辺のラベルを書き込んで、全部書き込んだ後に適当に頂点 $1$ に書き込むようにしたが
  - 3世代の間の2辺のラベルが同一だとうまくいかないことに気づく
    - 意外とこれを満たすテストケースが多かったようで半分以上が`WA`だった
- よって頂点 $1$ に適当に $1$ を書き込み、子に親と辺のラベルが同一なら被らない適当な数（ $1$ or $2$ ）を、異なるならラベルを書き込むようにした
...
- $x^2 - y = m^2$ とおくと、 $y = (x-m)(x+m)$
- よってこれを満たす正整数 $x$ と非負整数 $m$ の組み合わせを探せばよいのだが、 $x,m$ 平面でプロットしてもなかなかカオスな図形が出来上がるだけであまり旨味がない
- むしろ $p = x-m, q = x+m$ と置いて $pq$ 平面でプロットすることにする
  - なお $x,m$ が整数になる必要十分条件は $p, q$ の偶奇が一致すること
  - 他に課される条件は $pq \leq N, p \leq q, p > 0$
- あとは $1 \leq p \leq \srqt{N}$ の範囲で（飛び飛びだが）格子点を数え上げればOK
  - $p = k$ で切ったときの下端は $q = p$ なので常に条件を満たすことを留意すべし

## [M-SOLUTIONS プロコンオープン (2019) D - Maximum Sum of Minimum](https://atcoder.jp/contests/m-solutions2019/tasks/m_solutions2019_d)
- $c_i$ を昇順にソートしておく
- 木の辺に対してスコアが決まることから、書き込んだ数値のうち少なくとも1つはスコアに反映されない
  - よって $c_N$ は絶対に反映できない
...
- 二分探索で解を求める
- ある $S$ が条件を満たすかどうかについては
  - 強連結成分分解をして頂点を縮約したうえでDAG順で最初になる頂点からDFSで探索したが
    - $O(N^3)$ が間に合うので普通にFloyd-Warshall法で良かった……
    - まぁ一応 $O(N^2)$ に落とせているので良しとしておこう
      - （公式解説）頂点を縮約したグラフ $G'$ を作らなくとも元のグラフ $G$ でDFSすればOKというのは盲点だったので覚えておく

## [ABC176 E - Bomber](https://atcoder.jp/contests/abc176/tasks/abc176_e)
- $i$ 行目に存在する爆破対象の数を $A_i$ 、 $j$ 列目に存在する爆破対象の数を $B_j$ とおく
- $A_i$ の降順で以下を行う
  - $B_j$ の降順で以下を行う
    - $(i, j)$ に爆破対象が存在しないとき
      - $A_i + B_j > \mathrm{ans}$ なら解を更新する
      - そうでないなら $j$ に関する探索を打ち切る
    - $(i, j)$ に爆破対象が存在するとき
      - $A_i + B_j - 1 > \mathrm{ans}$ なら解を更新する
- 内側のループは高々 $M$ 回しか回らないので計算量は $O(H+M)$

## [ABC105 C - Base -2 Number](https://atcoder.jp/contests/abc105/tasks/abc105_c)
- 上の桁から確定させていくかと思いきや、下の桁から確定させると一発のパターン
- $i = 0, 1, \dots$ の順に $|N|$ の下から $i$ ビット目が
  - $1$ なら $\mathrm{ans}_i$ を $1$ にして $N \gets N - (-2)^i$
    - この操作により $N = 0$ となったら終了する
  - $0$ なら $\mathrm{ans}_i$ を $0$ にする
- ↑の操作だと元から $N=0$ のとき $\mathrm{ans}$ が空文字列になってしまうので事前に弾いておく

## [ABC237 E - Skiing](https://atcoder.jp/contests/abc237/tasks/abc237_e)
- 単純に得られる楽しさの値を辺のコストとして最長パスを求めようとしても、Dijkstraが使えないことから`TLE`不可避（Bellman–Fordだと $O(NM)$ ）
- 最終的に標高差と道中でどれだけ登ったかさえわかれば楽しさが求められるので
  - 登る方向は標高差の絶対値をコストとする辺、下る方向はコスト $0$ の辺を張ったグラフ上で頂点 $1$ から頂点 $i$ への最短路 $A_i$ を求める（負辺が無いのでDijkstraでOK）
    - このとき、広場 $1$ から広場 $i$ へ移動するときの楽しさの最大値は $H_1 - H_i - A_i$ で表せる
- 解は勿論 $\max H_1 - H_i - A_i$

## [ABC157 D - Friend Suggestions](https://atcoder.jp/contests/abc157/tasks/abc157_d)
- 人 $i$ の友人候補の数は頂点 $i$ が属する連結成分内で頂点 $i$ と直接繋がっていない頂点数に等しい
  - ただし連結成分内でブロック関係にある頂点の分だけ候補が減る
- よってDFSで連結成分ごとに以下を行う
  - 連結成分に属する頂点 $v$ ごとに以下を行う
...
- $p_i, d_i$ に対し、
  - $p_i$ から距離 $d_i$ 未満の頂点は必ず白く塗る必要がある
  - $p_i$ からちょうど距離 $d_i$ の頂点のうち少なくとも1つは黒く塗る必要がある
- 以降、これらの制約を条件 $i$ と書くことにする
- 全条件により白く塗る必要のある頂点を洗い出してから、各条件 $i$ に対して以下を行う
  - 少なくとも1つは黒く塗る必要がある頂点群を挙げ、それらの中に白く塗る必要のない頂点があれば全て黒く塗る
    - 白く塗る必要のない頂点が1個もない場合は`No`を出力する
- 各条件ごとにBFSを行うことにより、計算量は $O(K(N+M))$
...
## [ARC106 C - Solutions](https://atcoder.jp/contests/arc106/tasks/arc106_c)
- 1個の長い区間の中に $M+1$ 個の互いに交わらない区間を置くことで条件を達成できる
  - 逆に2つのプログラムの出力を異なるものにしようとすれば↑の状態を作らざるを得ず、このとき高橋くんのプログラムのほうが大きい値を出力することから $M<0$ は無条件で不可能とわかる
- $M>0$ のとき青木くんのプログラムは最低でも $1$ を出力することから、 $M \geq N-1$ のときは条件を達成できない
  - なお $M>0$ を考慮しそこねて1敗
    - 今回はテストケース名に`N1M0`の記述があったのでネタバレを食らった恰好になったが、これ自力で気づけるかなぁ……

## [diverta 2019 Programming Contest 2 C - Successive Subtraction](https://atcoder.jp/contests/diverta2019-2/tasks/diverta2019_2_c)
- ただ1つ残る整数は $A_i$ のうち $1$ 個以上 $N-1$ 個以下の符号を反転したときの和に限られる
  - よって基本的には $A_i < 0$ となるもののみ符号を反転すれば最大になる
- 実際に引き算する様子も答えるために以下の手順で解を求める
    1. $A_i$ を昇順にソートし、 $A_i < 0$ を満たす $i$ の個数を $k$ とする
        - ただし上述の「 $1$ 個以上 $N-1$ 個以下」の制約を守るため、 $k = 0$ のときは $k = 1$ とし、 $k = N$ のときは $k = N-1$ とする
    1. $x = A_1$ とする
    1. $i = k+1, k+2, \dots, N-1$ に対し $x \gets x - A_i$ を行う
    1. $x \gets A_N - x$ とする
    1. $i = 2, 3, \dots, k$ に対し $x \gets x - A_i$ を行う
    1. $M = x$ として解を出力する 
- $A_1$ は必ず符号を反転し $A_N$ は必ず符号を反転しないことを利用して、引き算をまとめて行うことで処理を簡略化できた

## [ARC135 B - Sum of Three Terms](https://atcoder.jp/contests/arc135/tasks/arc135_b)
- $S_{i+1} - S_i = A_{i+3} - A_i$ より、 $k \geq 2$ に対して以下が成り立つ
  - $A_{3k-2} = A_1 + \sum_{i=1}^{k-1} (S_{3i-1} - S_{3i-2})$
  - $A_{3k-1} = A_2 + \sum_{i=1}^{k-1} (S_{3i} - S_{3i-1})$
  - $A_{3k} = A_3 + \sum_{i=1}^{k-1} (S_{3i+1} - S_{3i})$
- よって $A_i \geq 0$ を満たすために以下が必要となる
  - $S_{3i-1} - S_{3i-2}$ の累積和の最小値 $m_1$ に対し、 $A_1 \geq M_1 = \max \{ 0, -m_1 \}$
  - $S_{3i} - S_{3i-1}$ の累積和の最小値 $m_2$ に対し、 $A_2 \geq M_2 = \max \{ 0, -m_2 \}$
  - $S_{3i+1} - S_{3i}$ の累積和の最小値 $m_3$ に対し、 $A_3 \geq M_3 = \max \{ 0, -m_3 \}$
- $M_1 + M_2 + M_3 \leq S$ なら条件を満たす数列 $A$ は存在する
  - $A_1 = M_1, A_2 = M_2, A_3 = S_1 - A_1 - A_2$ として、 $A_{i+3} = S_{i+1} - S_i + A_i$ より各項を求める
- $M_1 + M_2 + M_3 > S$ なら条件を満たす数列 $A$ は存在しない
- 要するに $i \bmod 3$ が等しい項のみ取り出したときに階差数列になっていることに気づけるかどうかがポイントだった

## [ABC307 E - Distinct Adjacent](https://atcoder.jp/contests/abc307/tasks/abc307_e)
- 輪状になっていなければ単に $M \cdot (M-1)^{N-1}$ 通りで済むが、それだと人 $N$ と人 $1$ が同じ数を貰う可能性がある
- よって人 $i$ が人 $1$ と同じ数を貰っているかどうかで場合分けをすればうまくいきそうなので
  - $\mathrm{dp}_{i,j}$ を先頭 $i$ 人に条件を満たすように数を渡すときに、人 $i$ が人 $1$ と異なる/同じ数を持つような場合の数とした
...
- 題意より $A_i = k_i \cdot 5^{x_i} \cdot 2^{y_i}$ （ $k_i$ は $2$ および $5$ と互いに素な正整数、 $x_i, y_i$ は整数）と表すことができ、このとき解は $i < j$ かつ $x_i + x_j \geq 0$ かつ $y_i + y_j \geq 0$ を満たす $(i, j)$ の個数になる
  - よって以下の手順で解が得られる
      1. $c_{x,y}$ を全て $0$ で初期化する
      1. $i = 1, 2, \dots, N$ の順に以下を行う
          1. ${\displaystyle \sum_{x \geq -x_i} \sum_{y \geq -y_i} c_{x,y}}$ を $\mathrm{ans}$ に加える
          2. $c_{x_i, y_i}$ に $1$ を加える
- $x_i, y_i$ の取りうる範囲について考える
  - 小数点以下が最大 $9$ 桁なのでひとまず $-9 \leq x_i, -9 \leq y_i$ は確定
  - $k_i = 1, y_i = -9$ のときに $x_i$ は最大となり、 $A_i < 10^4$ より $x_i \leq 9$ が得られる
  - 同様に $y_i \leq 34$ が得られる
...
- $B_j$ の単調増加区間（ $X_i > 0$ ）においては区間の途中で $A_j$ が極大になることはない
...
## [ABC242 E - (∀x∀)](https://atcoder.jp/contests/abc242/tasks/abc242_e)
- $M = \left\lfloor \frac{N}{2} \right\rfloor$ として
  - $X[1:M] < S[1:M]$ なら $X$ は条件を満たす文字列であることが確定する
  - $X[1:M] > S[1:M]$ なら $X$ は条件を満たさない文字列であることが確定する
  - $X[1:M] = S[1:M]$ のとき
    - $X$ が回文であることから $N$ 文字とも確定するため、単純に $X \leq S$ が成り立つかどうかを確かめればよい
- $X[1:M] < S[1:M]$ を満たす $X$ の個数は26進数の要領で数えられる

## [ABC269 F - Numbered Checker](https://atcoder.jp/contests/abc269/tasks/abc269_f)
- 行ごとに見ると公差 $2$ の等差数列であり、 $A_i, B_i, C_i, D_i$ により初項も項数も $O(1)$ で求まる
- 偶数行と奇数行に分けて見ると
  - 偶数行, 奇数行におけるその行の項数はそれぞれ等しい
  - ある偶数行の初項に $2M$ を足すと次の偶数行の初項になる（奇数行も同様）
  - よって各行の和は（偶数行、奇数行のそれぞれで）等差数列を成す
- 以上によりクエリあたり $O(1)$ で処理できる
...
## [ARC174 D - Digit vs Square Root](https://atcoder.jp/contests/arc174/tasks/arc174_d)
- 試しに $N = 10^6$ で条件を満たす $x$ を小さい方から列挙すると
  - $1$
  - $80, 90, 91, \dots, 99$
  - $100, 101, \dots, 109$
  - $9800, 9900, 9901, \dots, 9999$
  - $10000, 10001, \dots, 10099$
  - $998000, 999000, 999001, \dots, 999999$
- よって $x$ の桁数を $d$ とするとき、以下が成り立ちそうなのがわかる
...
- $H \leq 10$ より横方向の切断はあり得る $2^{H-1}$ 通りを全探索するとして
  - 各チャンクごとに横方向の累積和を取って貪欲に縦方向に切るべき場所を探していく
- 具体的にはチャンク $i$ の $j$ 列目までに含まれるホワイトチョコレートの個数を $S_{i,j}$ とすると
  - 前に切った箇所が $l$ のとき
    - 各 $i$ に対して $S_{i, l} + K \geq S_{i, r_i}$ を満たす最大の $r_i$ を求めた際の最小値が次に切るべき場所になる
      - なお次に切るべき場所が $l$ と等しくなってしまう場合は、そもそも横方向の切断パターンがマズいと判断する
      - 各列 $S_i$ に対して`upper_bound`で $S_{i,l} + K$ を探すのが最も実装が楽か
- 計算量は $O(2^H HW \log W)$ ……のはず
  - $2^{H-1}$ 回も $O(HW)$ のアロケーションをしているはずなのにわずか7msで通って震えた

## [ARC205 C - No Collision Moves](https://atcoder.jp/contests/arc205/tasks/arc205_c)
- 人 $i$ の移動を2点 $(S_i, 0), \ (T_i, 1)$ を結ぶ線分に対応させると
  - 2つの線分が交わるとき喧嘩は不可避になる
- よって人を $T_i$ の昇順に並べ替えておくと
  - 人 $i$ に関して、人 $j < i$ について $S_i < \max_j S_j$ が成り立つときはどのように移動しても喧嘩が発生する
  - $\max_j S_j < S_i$ ならば喧嘩が発生しない移動方法が存在する
- 喧嘩が不可避でなければ、右に移動する人を座標の大きい方から移動させ、左に移動する人を座標の小さい方から移動させればOK
- 計算量は $O(N \log N)$

## [ABC291 F - Teleporter and Closed off](https://atcoder.jp/contests/abc291/tasks/abc291_f)
- 都市 $k-M < i < k$ から都市 $k < j < k+M$ へ行けるテレポーターが存在すれば都市 $k$ を通らないルートが存在する
  - かつ、その際のテレポーターの使用回数の最小値は都市 $i$ までの使用回数の最小値と都市 $j$ より後の使用回数の最小値の和に1を加えたもの
- よってその2つを事前に求めておけばクエリ毎に $O(1)$ で答えられる

## [ABC246 E - Bishop 2](https://atcoder.jp/contests/abc246/tasks/abc246_e)
- $A_x + A_y \not\equiv B_x + B_y \pmod{2}$ のとき白マスビショップと黒マスビショップになるので $-1$
- マスに加えて直前の移動方向を情報に持てば、移動方向が変わらない辺のコストを $0$ 、変わる辺のコストを $1$ として頂点倍加01-BFSでOK
