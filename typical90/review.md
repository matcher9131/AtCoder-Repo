...
- 区別のつく $N$ 個を区別しない $K$ グループに分ける場合の数は $S(n, k)$
  - ただし $S(n, k)$ は第二種スターリング数
- 今回は最大でも $S(15, 6) \approx 4.2 \times 10^8$ なのでオーダー的にはギリギリ通る（？）
- バックトラッキングで実装。すなわち、 $i = 1, 2, \dots, N$ の順で
  - 既に頂点が存在するグループの個数を $c$ としてグループ $1, 2, \dots, c+1$ に頂点 $i$ を突っ込む
    - $c = K$ のときはグループ $c+1$ を考えない
    - 逆にまだグループ分けしていない頂点の個数 $N-i$ とまだ頂点を持たないグループの個数 $K-c$ が一致する場合はグループ $c+1$ のみを対象とする
- グループ内の距離最大値を事前計算するわけでもなくDFSの中でいちいちやっているのにもかかわらず普通に間に合う謎仕様
- （公式解説）あらかじめ $N$ 頂点から任意の組み合わせを選択したときの距離の2乗の最大値を計算しておくことで、「既に選んだ頂点集合」 「グループの個数」の2項目のみで解を求めることができる
  - すなわちどういう風にグループ分けしたのかの情報は必要ない
    - このとき $N$ 頂点の部分集合内でその部分集合を列挙することになるが、全体 $O(3^N)$ となる
